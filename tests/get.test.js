...
describe('get', () => {
    ...
    test('1: Valid string, target and position get 1', () => {
        const object = { 'a': [{ 'b': { 'c': 3 } }] };
        const result = get(object, 'a[0].b.c');
        expect(result).toEqual(3);
    });

    test('2: Valid string, target and position get 2', () => {
        const object = { 'a': [{ 'b': { 'c': 3 } }] };
        const result2 = get(object, ['a', '0', 'b', 'c']);
        expect(result2).toEqual(3);
    });
    
    test('3: Valid string, target and position get 3', () => {
        const object = { 'a': [{ 'b': { 'c': 3 } }] };
        const result3 = get(object, 'a.b.c', 'default');
        expect(result3).toEqual('default');
    });
    
    test('4: Valid string, target and position get 4', () => {
